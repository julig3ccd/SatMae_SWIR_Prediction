...
##### 3. provide real swir channel as target (pbbly rewrite the dataloader to provide the right target)
        ...
        if args.print_comparison:
              if idx % 100 == 0:
                save_swir_comparison_fig_from_tensor(swirpred,f'eval_comparison_fig_b_{idx}_p_{args.patch_size}',target_images=swir_targets,input=images)
                print('saved swir comparison figures for batch ',idx)
        if args.print_input: 
            if idx % 100 == 0:
                save_input_output_fig(final_swir_images=swirpred, name=f'input_output_fig_b_{idx}_p_{args.patch_size}', target_images=swir_targets, input=images)  
                print('saved input output figures for batch ',idx)   
