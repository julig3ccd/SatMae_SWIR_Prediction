...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler):
    ...
    if loss_scaler is not None:
        checkpoint_paths = [output_dir / ('checkpoint-pixel-wise-%s.pth' % epoch_name)]
...
def load_model(args, model_without_ddp, optimizer, loss_scaler):
    if args.resume:
        if args.resume.startswith('https'):
            ...
            #only delete keys if the exist on checkpoint
            for key in ['head.weight', 'head.bias']:
                if key in checkpoint['model']:
                    del checkpoint['model'][key]
            if args.eval == True and args.input_size != 96 and args.model != 'mae_vit_base_patch16':   
